...
mkdir -p $PKG_DIR
cd $PKG_DIR
git clone /var/local/jenkins/openmpi/pkgbuild
cd $PKG_DIR/pkgbuild/openmpi

# The openmpi repository is mounted into our Docker image, but read-only.
# We clone into a directory inside Docker (this is faster than cp).
git clone /var/local/jenkins/openmpi openmpi
...
$PKG_DIR/pkgbuild/openmpi/pkgbuild_jenkins.sh

ls -l *.deb

# The directory that is copied from Docker back into the Jenkins workspace.
COPY_FROM_DOCKER=/var/local/git/openmpi/testoutput
